package com.scaffold.boot3.common.util;
...
public class JWTUtil {
    ...
    public static String createToken(String userId) {
        return createToken(userId, Collections.emptyList());
    }

    public static String createToken(String userId, List<String> authority) {
        Map<String, Object> claims = new HashMap<>();

        claims.put("id", userId);
        claims.put("authList", authority);
        ...
        return Jwts.builder().setClaims(claims).setExpiration(dt).signWith(SignatureAlgorithm.HS512, JWT_SIGN_KEY).compact();
    }
    ...
    public static SessionUserDetails checkToken(String token) {
        ...
        SessionUserDetails user = new SessionUserDetails(id, setAuthorities(auth_list));
        // 80%이상 만료 시 자동 재발급
        // 응답 헤더를 받은 사용자는 갱신 요청을 보내야한다.
        if (expiresInMs <= expiredThreshold) {
            user.setToken(createToken(id, auth_list));
        }
